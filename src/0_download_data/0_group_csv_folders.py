...
# %%
def read_all_csv_in_folder_no_sort(path, output_folder, file_name):
    csv_paths = [os.path.join(path, f) for f in os.listdir(path) if f.endswith(".csv")]
    final_df = pd.DataFrame()
...
read_all_csv_in_folder_no_sort(path, output_folder, file_name)

# %%
mal_user_manga_list = pd.read_csv("data/raw/mal_user_manga_list.csv")
len(set(mal_user_manga_list["user_index"].to_list()))
mal_user_manga_list.shape[0]

# %%
############## 11_anilist_api_manga_grab ###############
path = "data/raw/ani_data_tables/"
output_folder = "data/raw/"
file_name = "ani_data"
read_all_csv_in_folder(path, output_folder, file_name)
